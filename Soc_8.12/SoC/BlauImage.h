...
public:
	...
	BlauImage(void) :nChannels(0), Height(0), Width(0), nWStep(0), pImageData(NULL), roi(NULL) ,pixels(NULL){}//기본 생성자

	BlauImage(int width, int height, int channels = 1)    //할당생성자
		:nChannels(channels), Width(width), Height(height), 
		nWStep(((Width*nChannels * sizeof(T) + 3)&~3) / sizeof(T)), roi(NULL), pixels(NULL)
	{
	...
	void Flip() const
	{
		...
		for (int r = 0; r<Height; r++)
		{
			memcpy(pSrc, pDst, nWStepS*sizeof(T));
			pDst += nWStepD;
			pSrc += nWStepS;
		}
	...
	void CreateImage(int w, int h){
		...
		memset(pixels[0], 0, sizeof(T) * Width * Height); //픽셀값 초기화
	}

	template<typename U> void Convert(const BlauImage<U>& img, bool use_limit = false){
