...
{
  AA = cvCreateMat(8, 9, CV_32FC1);//cv2cpp
...
bool homography_estimator::estimate_from_inliers(homography06 * H)
{
  ...
  CvMat * _AA_ = cvCreateMat(2 * number_of_inliers, 9, CV_32FC1);//cv2cpp
  ...
  if(number_of_inliers == 4) {
      cvSVD(_AA_, W8, 0, Vt, CV_SVD_MODIFY_A | CV_SVD_V_T);//cv2cpp
  }
  else {
