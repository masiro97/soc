...
bool manage_image(ByteImage ** image, int width, int height, int depth, int nChannels)
{
  bool just_created = false;

  if (*image == 0) {
    *image = cvCreateImageHeader(B_size(width, height), depth, nChannels);
    (*image)->imageData = 0;
    just_created = true;
  }
  ...
  if (just_created || different) {
    ...
    (*image)->GetWStep() = width_step;
    (*image)->imageSize = image_size;

    (*image)->GetWidth()     = width;
    (*image)->GetHeight()    = height;
    (*image)->depth     = depth;
    (*image)->GetChannel() = nChannels;
...
{
  ByteImage * tmp_image = mcvLoadImage(filename, code);

  if (tmp_image == 0) return false;

  manage_image(image, tmp_image->GetWidth(), tmp_image->GetHeight(), tmp_image->depth, tmp_image->GetChannel());
  tmp_image = *image;
...
{
  f << image->GetWidth() << " " << image->GetHeight() << " " << image->depth << " " << image->GetChannel()
    << " " << image->GetWStep() << " " << image->imageSize << endl;
