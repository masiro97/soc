...
planar_pattern_detector::~planar_pattern_detector(void)
{
  if (model_image != 0) cvReleaseImage(&model_image);//cv2cpp
...
{
  if (input_image->GetChannel() != 1 || input_image->depth != IPL_DEPTH_8U) {
...
{
  ByteImage * color_model_image = mcvGrayToColor(model_image);//cv2cpp
