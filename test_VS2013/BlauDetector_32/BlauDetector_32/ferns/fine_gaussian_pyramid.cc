...
fine_gaussian_pyramid::fine_gaussian_pyramid(int type, char * image_name, int outer_border, int number_of_octaves, int inner_border)
{
  ...
  ByteImage * image = mcvLoadImage(image_name, 0);
  alloc(image->GetWidth() - 2 * inner_border, image->GetHeight() - 2 * inner_border, outer_border, number_of_octaves, inner_border);
  set_image(image);
...
void fine_gaussian_pyramid::set_image(const ByteImage * image)
{
  ...
  tmp_image = aztec_pyramid[0];
  cvFree(&(aztec_pyramid[0]->roi));//cv2cpp
  ...
  for(int i = outer_border + image->GetHeight(); i < total_height; i++) {
    unsigned char * border = mcvRow(aztec_pyramid[0], i, unsigned char);
    unsigned char * last_row = mcvRow(image, image->GetHeight() - 1, unsigned char);
  ...
  for(int i = outer_border + image->GetWidth(); i < total_width; i++) {
    unsigned char * border = mcvRow(aztec_pyramid[0], 0, unsigned char) + i;
    unsigned char * first_row = mcvRow(aztec_pyramid[0], 0, unsigned char) + image->GetWidth() + outer_border - 1;
    for(int j = 0; j < total_height; j++) {
      *border = *first_row;
      border += j_step;
      first_row += j_step;
...
void fine_gaussian_pyramid::expand(ByteImage * original_image, ByteImage * final_image, int n)
{
  ...
  if (add_a_row[n - 1])
    {
      unsigned char * dest1 = mcvRow(final_image, 2 * original_image->GetHeight() - 1, unsigned char);
      unsigned char * dest2 = mcvRow(final_image, 2 * original_image->GetHeight(), unsigned char);
