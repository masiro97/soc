...
{
  f << image->width << " " << image->height <<" " << image->nChannels
    << " " << image->widthStep  << endl;

  int image_size = image->widthStep *image->height* image->nChannels;
  save_buffer_in_pakfile(f, image->imageData, image_size, BYTE);
}
...
{
	int width, height, depth, nChannels, width_step;
	depth = IPL_DEPTH_32F;
