...
#define PI 3.14
#define B_Row(image, y, type) ((type  *)((image)->GetPtr() + (y)* (image)->GetWStep()))

//void DrawLines(const ByteImage& ImageIn, int nTNum, int nTVal, double resTheta, int numLine, int* pRho, int* pTheta);
...
public:
	...
	B_Mat(int Rows, int Cols) {
		rows = Rows; cols = Cols;
		float* dat = new float[rows*cols]; //(float*)malloc(sizeof(float)*rows*cols);
		data.fl = dat;
	}
	B_Mat(void) {};
	//~B_Mat();
...
public:
	...
	BlauImage<T> Flip() const
	{
		BlauImage<T> dst(Width, Height, nChannels);

		T* pSrc = pImageData;
		T* pDst = dst.GetPtr(Height - 1, 0);
		...
		for (int r = 0; r<Height; r++)
		{
			memcpy(pDst, pSrc, nWStepD*sizeof(T));
			pDst -= nWStepD;
			pSrc += nWStepS;
		}
	...
	BlauImage& operator=(const BlauImage& img)
	{
		...
		if (roi == NULL) {
			...
			if (roi->coi == -1) nChannels = img.nChannels; else nChannels = 1;
			Height = roi->height;	Width = roi->width;	nWStep = ((Width*nChannels*sizeof(T) + 3)&~3) / sizeof(T);
			...
			{
				pImageData = new T[Height*nWStep];
				T* psrc = img.pImageData;
				psrc += roi->xoffset + roi->yoffset*img.nWStep;

				for (int r=0;r<Height;r++)
				{
					memcpy(pImageData, psrc, nWStep * sizeof(T));
					pImageData += nWStep;
					psrc += img.nWStep;
				}
			}
...
void B_SVD(B_Mat* A, B_Mat* W, B_Mat* U, B_Mat* V);
...
ByteImage AutoContrastControl(ByteImage *a_ImageIn);
