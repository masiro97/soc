									    ...
									    int roi_bottom_right_u, int roi_bottom_right_v)
{
  ...
  {
	  detector->kload(keypoint_filename);
								 ...
								 int roi_bottom_right_u, int roi_bottom_right_v)
{
  ...
  if (keyf.good())
  {
	  cout << "> [planar_pattern_detector_builder::learn] Reading Keypoint from file " << keypoint_filename << ".\n";
	  detector -> kload(keypoint_filename);
  }
  else {
	  ...
	  detector -> ksave(keypoint_filename);
  }
