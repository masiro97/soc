...
inline void pyr_yape06::compute_laplacian(ByteImage * smoothed_image, ByteImage * laplacian,
					  const int w, const int h,
					  const int Dxx, const int Dyy,
					  const int /*Dxy*/, const int /*Dyx*/)
{
  for(int y = Dxx; y < h - Dxx; y++) {
    unsigned char * image_row = B_Row(smoothed_image, y, unsigned char);//
...
{
  unsigned char * image_ptr = B_Row(smoothed_image, y, unsigned char) + x;
