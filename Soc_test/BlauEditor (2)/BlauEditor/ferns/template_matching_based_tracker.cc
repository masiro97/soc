...
bool template_matching_based_tracker::load(const char * filename)
{
  ...
  I0 = &B_CreateMat(nx * ny, 1);
  i0 = I0->data.fl;

  for(int i = 0; i < nx * ny; i++)
    f >> i0[i];

  I1 = &B_CreateMat(nx * ny, 1);
  i1 = I1->data.fl;
  DI = &B_CreateMat(nx * ny, 1);
  DU = &B_CreateMat(8, 1);
  du = DU->data.fl;

  f >> number_of_levels;

  As = new B_Mat*[number_of_levels];
  for(int i = 0; i < number_of_levels; i++) {
    As[i] = &B_CreateMat(8, nx * ny);
    for(int j = 0; j < 8; j++)
...
void template_matching_based_tracker::add_noise(B_Mat * V)
{
  float * v = V->data.fl;
...
ByteImage * template_matching_based_tracker::compute_gradient(ByteImage * image)
{
  ByteImage * dx = B_CreateImage(B_size(image->GetWidth(), image->GetHeight()),1);
  ByteImage * dy = B_CreateImage(B_size(image->GetWidth(), image->GetHeight()),1);
  ByteImage * result = B_CreateImage(B_size(image->GetWidth(), image->GetHeight()),1);
  B_Sobel(image, dx, 0);//ori ==0 -> x, ==1 -> y
  B_Sobel(image, dy, 1);
  MulImage(*dx, *dx, *dx);//cv2cpp
  MulImage(*dy, *dy, *dy);
  AddImage(*dx, *dy, *result);//cv2cpp
...
void template_matching_based_tracker::get_local_maximum(ByteImage * G,
							int xc, int yc, int w, int h,
							int & xm, int & ym)
{
  int max = -1;
  for(int v = yc - h / 2; v <= yc + h / 2; v++) {
    short * row = B_Row(G, v, short);
...
void template_matching_based_tracker::find_2d_points(ByteImage * image, int bx, int by)
{
  ByteImage * gradient = compute_gradient(image);
  ...
  B_ReleaseImage(&gradient);
}
/*
void template_matching_based_tracker::compute_As_matrices(ByteImage * image, int max_motion, int Ns)
{
  ...
  for(int level = 0; level < number_of_levels; level++) {
	...
	i1[i] = mcvRow(image, y1, unsigned char)[x1];//cv2cpp
      }
      add_noise(I1);
    ...
    cvGEMM(H, H, 1.0, 0, 0.0, HHt, CV_GEMM_B_T);//cv2cpp
    cout << "done." << endl;

    cout << " - inverting HHt..." << flush;
    if (cvInvert(HHt, HHt_inv, CV_SVD_SYM) == 0) {//cv2cpp
    ...
    cvMatMul(Y, Ht_HHt_inv, As[level]);//cv2cpp
    cout << "done." << endl;
  }
...
void template_matching_based_tracker::learn(ByteImage * image,
...
bool template_matching_based_tracker::track(ByteImage * input_frame)
{
  homography06 fs;
  ...
  for(int level = 0; level < number_of_levels; level++) {
	...
	if (x1 < 0 || y1 < 0 || x1 >= input_frame->GetWidth() || y1 >= input_frame->GetHeight())
	  return false;

	i1[i] = B_Row(input_frame, y1, unsigned char)[x1];
      }
      normalize(I1);
      B_Sub(I1, I0, DI);//cv2cpp

      B_MatMul(As[level], DI, DU);
      he.estimate(&fs,
