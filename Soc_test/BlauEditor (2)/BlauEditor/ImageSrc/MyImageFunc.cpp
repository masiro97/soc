...
double m_bufGss[17];
void B_Gaussian5x5(ByteImage* ImgIn, ByteImage* ImgOut)
{
	int nWidth = ImgIn->GetWidth();
	int nHeight = ImgIn->GetHeight();
	int nChnnl = ImgIn->GetChannel();
...
void B_GaussianSmoothing_7x7(ByteImage* src, ByteImage* dst, ByteImage* int_buffer)
{
...
void B_PyrDown(ByteImage* src, ByteImage* dst)
{
	...
	int wstep1 = src->nWStep, wstep2 = src->nWStep;
	BYTE *psrc = src->pImageData, *pdst = dst->pImageData;

	B_Gaussian5x5(src, src);

	int i_dst = 0;
	for (int i = 0; i < wstep1*height1; i += 2) {
		pdst[i_dst] = psrc[i];
		i_dst++;
	}
}
