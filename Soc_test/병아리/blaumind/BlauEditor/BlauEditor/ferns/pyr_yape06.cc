...
pyr_yape06::~pyr_yape06(void)
{
  ...
  delete_managed_buffer((laplacian).GetPtr());
  delete &laplacian;
}
...
void pyr_yape06::compute_Ds(ByteImage * smoothed_image)
{
  Dxx = R, Dyy = R * smoothed_image->GetWStep();
  Dxy = Rp + Rp * smoothed_image->GetWStep(), Dyx = Rp - Rp * smoothed_image->GetWStep();
  DXY = 1  + 1  * smoothed_image->GetWStep(), DYX = 1  - 1  * smoothed_image->GetWStep();
}

inline void pyr_yape06::compute_laplacian(ByteImage * smoothed_image, IntImage * laplacian,
					  const int w, const int h,
					  const int Dxx, const int Dyy,
					  const int /*Dxy*/, const int /*Dyx*/)
{
  for(int y = Dxx; y < h - Dxx; y++) {
    BYTE * image_row = B_Row(smoothed_image, y, BYTE);//

    int * laplacian_row = B_Row(laplacian, y, int);
...
void pyr_yape06::compute_laplacian(ByteImage * smoothed_image)
{
  //manage_image(&laplacian, smoothed_image->GetWidth(), smoothed_image->GetHeight(), smoothed_image->nChannels);//cv2cpp
	laplacian = IntImage(smoothed_image->GetWidth(), smoothed_image->GetHeight(), smoothed_image->nChannels);
	laplacian.SetConstValue(0);

  const int w = smoothed_image->GetWidth();
  const int h = smoothed_image->GetHeight();
  if (w == 784 && h == 640)
...
inline int pyr_yape06::hessian_min_eigen_value(ByteImage * smoothed_image, const int tr, const int x, const int y)
{
  BYTE * image_ptr = B_Row(smoothed_image, y, BYTE) + x;
...
inline bool pyr_yape06::laplacian_hessian_criteria(ByteImage * laplacian,
						   const int x, const int y)
{
  int * image_ptr = B_Row(laplacian, y, int) + x;
  const int D = laplacian->GetWStep() / 4;
...
void pyr_yape06::add_local_extrema(fine_gaussian_pyramid * pyramid, ByteImage * smoothed_image, int scale)
{
  const int w = laplacian.GetWidth();
  const int dy = laplacian.GetWStep() / sizeof(int);
  ...
  for(int y = 1; y < laplacian.GetHeight() - 1; y++) {
    int * lap_row = B_Row(&laplacian, y, int);
    for(int x = 1; x < w - 1; x++) {
...
/*
ByteImage * pyr_yape06::draw_keypoints(fine_gaussian_pyramid * pyramid, keypoint * keypoints, int number_of_keypoints)
{
  ByteImage * result = mcvGrayToColor(pyramid->original_image);

  for(int i = 0; i < number_of_keypoints; i++)
    cvCircle(result, mBlauPoint(keypoints[i]), 16 << int(keypoints[i].scale), mcvRainbowColor(int(keypoints[i].scale)));
