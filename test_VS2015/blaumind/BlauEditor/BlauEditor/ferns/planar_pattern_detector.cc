...
planar_pattern_detector::~planar_pattern_detector(void)
{
  if (model_image != 0) delete model_image;//cv2cpp
...
bool planar_pattern_detector::kload(const char * filename)
{
	ifstream f(filename, ios::binary);

	if (!f.is_open()) return false;

	cout << "> [planar_pattern_detector::kload] Loading keypoint file " << filename << " ... " << endl;

	bool result = kload(f);

	f.close();

	cout << "> [planar_pattern_detector::kload] Ok." << endl;

	return result;
}

bool planar_pattern_detector::ksave(const char * filename)
{
	ofstream f(filename, ios::binary);

	if (!f.is_open()) {
		cerr << ">! [planar_pattern_detector::ksave] Error saving file " << filename << "." << endl;
		return false;
	}

	cout << "> [planar_pattern_detector::ksave] Saving keypoint file " << filename << " ... " << flush;
...
bool planar_pattern_detector::kload(ifstream & f)
{
	f >> number_of_model_points;
	cout << "> [planar_pattern_detector::kload] " << number_of_model_points << " model points." << endl;
...
bool planar_pattern_detector::ksave(ofstream & f)
{
...
bool planar_pattern_detector::detect(const ByteImage * input_image)
{
  if (input_image->GetChannel() != 1) {
    cerr << ">! [planar_pattern_detector::detect] Wrong image format" << endl;
    cerr << ">! nChannels = " << input_image->GetChannel()
	  << "." << endl;
...
void planar_pattern_detector::save_image_of_model_points(ByteImage* src, int patch_size)
{

	for (int i = 0; i < number_of_detected_points; i++)
	{
		BlauCircle(src,
			BlauPoint(int(detected_points[i].fr_u() + 0.5), int(detected_points[i].fr_v() + 0.5)), patch_size / 2 * (1 << int(detected_points[i].scale)),
			BlauScalar(0, 255, 0));
	}
}
...
IplImage * planar_pattern_detector::create_image_of_matches(void)
{
  ...
  for(int i = 0; i < number_of_model_points; i++)
    mcvCross(result, model_points[i].fr_u(), model_points[i].fr_v(), 5, cvScalar(255, 0, 0), 1);//cv2cpp
