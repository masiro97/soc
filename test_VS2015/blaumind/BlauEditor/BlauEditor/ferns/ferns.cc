...
{
  if (pyramid->full_images[level].GetWidth()  != width_full_images ||
      pyramid->full_images[level].GetHeight() != height_full_images) {
    precompute_D_array(D_full_images, &pyramid->full_images[level]);
    width_full_images  = pyramid->full_images[level].GetWidth();
    height_full_images = pyramid->full_images[level].GetHeight();
  }

  ByteImage * smoothed_image = &pyramid->full_images[level];
  int shift_x = x + pyramid->border_size;
  int shift_y = y + pyramid->border_size;

  if (shift_x < max_d || shift_y < max_d || shift_x >= smoothed_image->GetWidth() - max_d  || shift_y >= smoothed_image->GetHeight() - max_d)
    return false;

  unsigned char * C = (unsigned char *)(smoothed_image->GetPtr() +
                                        shift_y * smoothed_image->GetWStep() +
                                        shift_x);
...
bool ferns::drop_aztec_pyramid(fine_gaussian_pyramid * pyramid, int x, int y, int level, int * leaves_index)
{
  ...
  if (pyramid->aztec_pyramid[level].GetWidth()  != width_aztec_pyramid[octave] ||
      pyramid->aztec_pyramid[level].GetHeight() != height_aztec_pyramid[octave]) {
    precompute_D_array(D_aztec_pyramid[octave], &pyramid->aztec_pyramid[level]);
    width_aztec_pyramid[octave]  = pyramid->aztec_pyramid[level].GetWidth();
    height_aztec_pyramid[octave] = pyramid->aztec_pyramid[level].GetHeight();
  }

  ByteImage * smoothed_image = &pyramid->aztec_pyramid[level];
...
void ferns::precompute_D_array(int * D, ByteImage * image)
{
  for(int i = 0; i < number_of_ferns; i++)
    for(int j = 0; j < number_of_tests_per_fern; j++) {
      int k = i * number_of_tests_per_fern + j;
      D[2 * k]     = DX1[k] + image->GetWStep() * DY1[k];
      D[2 * k + 1] = DX2[k] + image->GetWStep() * DY2[k];
    }
}
