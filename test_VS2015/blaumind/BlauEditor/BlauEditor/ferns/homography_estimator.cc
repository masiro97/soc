...
{
  float inv_H33 = 1.f / float( cvmGet(H, 2, 2) );//cv2cpp
  for(int i = 0; i < 8; i++)
    H->data.fl[i] *= inv_H33;
  cvmSet(H, 2, 2, 1.);//cv2cpp
}
...
bool homography_estimator::estimate_from_inliers(homography06 * H)
{
  ...
  B_Mat  _AA_ = B_CreateMat(2 * number_of_inliers, 9);//cv2cpp
