...
class planar_pattern_detector
{
 public:
  ...
  bool kload(const char * keypoint_data_filename);
  bool ksave(const char * keypoint_data_filename);
  bool kload(ifstream & f);
  bool ksave(ofstream & f);
  ...
  void save_image_of_model_points(ByteImage* src, int patch_size);

  //! set the maximum number of points we want to detect
  void set_maximum_number_of_points_to_detect(int max);

  bool detect(const ByteImage * input_image);

 // ByteImage * create_image_of_matches(void);
  ...
  //private:
  void learn(ByteImage * model_image,
