...
fine_gaussian_pyramid::fine_gaussian_pyramid(int type, int outer_border, int number_of_octaves, int inner_border)
{
  ...
  intermediate_int_image = IntImage();
}
...
void fine_gaussian_pyramid::alloc(int width, int height, int outer_border, int number_of_octaves, int inner_border)
{
  ...
  original_image = ByteImage(width + 2 * inner_border, height + 2 * inner_border, 1);
  intermediate_int_image = IntImage(total_width, total_height, 1);
  ...
  case yape_pyramid_3: {
	...
	c /= 2;
      ...
      if (i % 4 == 1)
	 aztec_pyramid[i]=ByteImage(octave_total_width, octave_total_height, 1);
      else
	aztec_pyramid[i] = ByteImage();
      full_images[i] = ByteImage();
      coeffs[i] = c;
    }
    break;
  }
  ...
  case full_pyramid_357:
	...
	full_images[i] = ByteImage(total_width, total_height, 1);
	aztec_pyramid[i] = ByteImage(octave_total_width, octave_total_height, 1);
	coeffs[i] = c;
      }
      break;
...
void fine_gaussian_pyramid::set_image( ByteImage * image)
{
  ...
  B_Copy(tmp_image, aztec_pyramid);
  ...
  for(int i = 0; i < outer_border; i++) {
    unsigned char * border = B_Row(aztec_pyramid, i, unsigned char);
  ...
  for(int i = 0; i < outer_border; i++) {
    unsigned char * border = B_Row(aztec_pyramid, 0, unsigned char) + i;
    unsigned char * first_row = B_Row(aztec_pyramid, 0, unsigned char) + outer_border;
  ...
  for(int i = outer_border + image->GetWidth(); i < total_width; i++) {
    unsigned char * border = B_Row(aztec_pyramid, 0, unsigned char) + i;
    unsigned char * first_row = B_Row(aztec_pyramid, 0, unsigned char) + image->GetWidth() + outer_border - 1;
    for(int j = 0; j < total_height; j++) {
      *border = *first_row;
      border += j_step;
      first_row += j_step;
...
void fine_gaussian_pyramid::compute_from_level0(void)
{
  ...
  case yape_pyramid_7:
    for(int i = 0; i < number_of_octaves; i++) {
		B_Gaussian(&(aztec_pyramid[4 * i]), &(aztec_pyramid[4 * i + 3]), 7);
		//ShowImage(aztec_pyramid[4 * i]);
     // B_GaussianSmoothing_7x7(&(aztec_pyramid[4 * i]), &(aztec_pyramid[4 * i + 3]), &intermediate_int_image);;
	//  ShowImage(aztec_pyramid[4 * i+3]);
